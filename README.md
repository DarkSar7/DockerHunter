...
- **Output & Context Customization**:
  - `--output`, `-o`: Standardizes output files under a parent directory named `dockerhunter/` (e.g., `dockerhunter/result.json`). If a directory path is specified, it creates the `dockerhunter/` folder inside it.
  - `--pre`: Saves candidates matching signature rules to `pre.json` *before* they are sent to the AI validator (e.g., `dockerhunter/pre.json`).
  - `--context <mode>`: Context export mode to extract verified source code from images. The repository folder is only created if the AI validates at least one secret finding. Slashes are preserved, but registry domains (like `index.docker.io/`) are stripped. Modes are:
    - `none` (default): No context exported.
    - `files`: Exports only the source files containing validated secrets into `dockerhunter/<repo_path>/`, preserving the exact hierarchy.
...
Extract only source files containing validated secrets under `output/dockerhunter/<repo_path>/`:
```bash
...
```
*(Note: If a scanned image contains zero validated secrets, the `<repo_path>/` directory will not be created at all).*
